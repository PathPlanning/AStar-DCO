...
bool Config::getConfig(const char *FileName)
{
    ...
    tinyxml2::XMLElement *root = 0, *algorithm = 0, *element = 0, *options = 0, *structure = 0;
    ...
    structure = root->FirstChildElement(CNS_TAG_STR);
    OpenStructure = new int[2];
    ...
    {
        element = structure->FirstChildElement(CNS_TAG_OT);
        if (!element)
        {
            std::cout << "Warning! No '" << CNS_TAG_OT << "' tag found in XML file." << std::endl;
            std::cout << "Value of '" << CNS_TAG_OT << "' tag was defined to 'vectoroflists' (0)." << std::endl;
            OpenStructure[CN_OS_OT] = CN_OS_OT_VL;
        }
        else
        {
            stream << element->GetText();
            ...
            stream.clear();
            std::transform(value.begin(), value.end(), value.begin(), ::tolower);
            if(value == CNS_OS_OT_VL)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_VL;
            }
            else if(value == CNS_OS_OT_LS)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_LS;
            }
            else if(value == CNS_OS_OT_ST)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_ST;
            }
            else if(value == CNS_OS_OT_PQ)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_PQ;
            }
            else if(value == CNS_OS_OT_VP)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_VP;
            }
            else if(value == CNS_OS_OT_VS)
            {
                OpenStructure[CN_OS_OT] = CN_OS_OT_VS;
            }
            else
            {
                std::cout << "Warning! Value of '" << CNS_TAG_OT << "' is not correctly specified" << std::endl;
                std::cout << "Value of '" << CNS_TAG_OT << "' tag was defined to 'vectoroflists' (0)." << std::endl;
                OpenStructure[CN_OS_OT] = CN_OS_OT_VL;
            }
        }
        ...
        else
        {
            ...
            {
                std::cout << "Value of '" << CNS_TAG_OT << "' allows only duplicates" << std::endl;
                std::cout << "Value of '" << CNS_TAG_OD << "' was defined to true." << std::endl;
